...
arma::vec::fixed<3> Inverse_projection_model(cv::Point2d uvd,int model_type,bool return_Jac,CAM camera_parameters, arma::mat::fixed<3,2>& dhc_duvd )
{
    ...
    uv = Undistort_a_point( uvd,camera_parameters,ditortion_model );  // undistort point
    ...
    if (return_Jac == true) // compute Jacobian
    {
      ...
      arma::mat::fixed<2,2> Jdp = Jac_Distort_a_point(uv,camera_parameters,ditortion_model);
...
cv::Point2d Projection_model(arma::vec::fixed<3> Pc,int model_type,bool return_Jac,CAM camera_parameters, arma::mat::fixed<2,3>& duv_dPc)
{
    ...
    {
      int ditortion_model = 1; 
      arma::mat::fixed<2,2> dh_uuud = Jac_Distort_a_point(uvd,camera_parameters,ditortion_model);
      double Pc1 = Pc(0);
      double Pc2 = Pc(1);
      double Pc3 = Pc(2);
...
//
// 2  --> to be implemented
arma::mat::fixed<2,2> Jac_Distort_a_point(cv::Point2d uv, CAM camera_parameters,int ditortion_model )
{
  //-------------------------------------------------------
